...
public class IPLayer implements BaseLayer {
   ...
   public synchronized void replyArrived(byte[] input, int length, byte[] bytes) { // 
          if (input == null) { // not receiving packet -> wait
              try {
                  wait(); // pause until the constructor thread give data

              } catch (InterruptedException e) {
              }
          }
          // received input(ARP Reply) -> receive mac address by reply
          
          // From ARP Layer , Receive ARP Reply & Store MacAddress that is in ARP Header
          String dst_mac_addr = ((ARPLayer) this.GetUnderLayer(0)).getMacAddr(Utils.convertAddrFormat(StaticRouterGUI.ARP_DEST_IP_ADDR));
          // Then Send to Ethernet Layer with DstMacAddr
          ...
          System.out.println("이 스레드가 받은 패킷: " + input);
          input = null; 
          notify(); // make paused constructor thread into wait-for-run state
   }
   ...
   public boolean Send(byte[] input, int length) {
      ...
      if (!dst_mac_addr.equals("IsNotExist")) {
         ...
         replyArrived(input, length, bytes);
      }
